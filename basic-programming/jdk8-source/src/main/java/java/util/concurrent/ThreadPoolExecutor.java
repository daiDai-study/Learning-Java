...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    // 所有任务都已终止, 工作线程数为0, 线程转化为TIDYING状态并准备调用terminated方法
     ...
     *
     * firstTask：如果指定了该参数, 表示将立即创建一个新工作线程执行该firstTask任务; 否则复用已有的工作线程，从工作队列中获取任务并执行
     *
     ...
     * state).
     *
     * core：执行任务的工作线程归属于哪个线程池:  true-核心线程池  false-非核心线程池
     *
     * @return true if successful
     */
    private boolean addWorker(Runnable firstTask, boolean core) {
        // 对线程池的状态进行一些判断，目的是确认当前线程池是否接受新任务并执行
        retry:
        for (;;) {
            int c = ctl.get();
            int rs = runStateOf(c);
             ...
             * 3. 线程池状态 == SHUTDOWN 、firstTask == null 但 队列为空: 队列中已经没有任务了, 所以也就不需要执行任何任务了，可以直接返回
             */
            ...
            for (;;) {
                ...
                if (wc >= CAPACITY ||
                    ...
                    return false;
                if (compareAndIncrementWorkerCount(c)) // 工作线程数成功加1，跳出最外层循环
                    break retry;
    ...
    private void processWorkerExit(Worker w, boolean completedAbruptly) {
        if (completedAbruptly) // If abrupt, then workerCount wasn't adjusted，工作线程因异常情况而退出
     ...
     *
     * 通过自旋，不断地尝试从阻塞队列中获取一个任务，如果获取失败则返回null
     *
     * @return task, or null if the worker must exit, in which case
     *         workerCount is decremented
     */
    private Runnable getTask() {
        boolean timedOut = false; // Did the last poll() time out? 表示上次从阻塞队列中取任务时是否超时
    ...
    final void runWorker(Worker w) {
        ...
        w.firstTask = null;
        w.unlock(); // allow interrupts，允许执行线程被中断
     ...
     *        executed.  This queue will hold only the {@code Runnable}
     *        tasks submitted by the {@code execute} method.
     *        任务队列, 保存已经提交但尚未被执行的线程
     * @param threadFactory the factory to use when the executor
     *        creates a new thread
     * @param handler the handler to use when execution is blocked
     *        because the thread bounds and queue capacities are reached
     *        拒绝策略 (当任务太多导致工作队列满时的处理策略)
     * @throws IllegalArgumentException if one of the following holds:<br>
     *         {@code corePoolSize < 0}<br>
     *         {@code keepAliveTime < 0}<br>
     *         {@code maximumPoolSize <= 0}<br>
    ...
    public void execute(Runnable command) {
        ...
        if (isRunning(c) && workQueue.offer(command)) {
            ...
            if (! isRunning(recheck) && remove(command))
                ...
                // 当成功插入任务至队列，且工作线程数为0时，需要创建一个空任务的工作线程，后续可从任务队列中取任务执行
                // 否则，该队列中的任务永远不会被执行
